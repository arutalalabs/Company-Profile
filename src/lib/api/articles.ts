...
export function transformArticleForDisplay(article: Article): DisplayArticle {
  return {
    ...
    created_date: article.created_at || article.created_date || new Date().toISOString(),
...
export async function getPublishedArticles(): Promise<DisplayArticle[]> {
  ...
  const publishedArticles = response.data.filter((article) => article.article_status === "PUBLISHED");
