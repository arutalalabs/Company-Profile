...
import type { UpcomingCourse, AvailableCourse, Contributor, Instructor, BatchSession, CourseDetail, CourseDetailBatch, Prices } from "@/types/course";
...
export async function getAvailableCourses(): Promise<AvailableCoursesResponse> {
  return apiGet<AvailableCoursesResponse>("/courses?available=true&isDisplayed=true");
}

export async function getCourseById(courseId: string): Promise<CourseDetailResponse> {
  const apiResponse = await apiGet<ApiCourseDetailResponse>(`/courses/${courseId}`, { revalidate: 60 });
  ...
  const transformedData: CourseDetail = {
    ...
    course_material:
      apiResponse.data.courseMaterial?.map((m) => ({
        title: m.title,
        description: m.description,
      })) || [],
    course_benefit:
      apiResponse.data.courseBenefit?.map((b) => ({
        title: b.title,
        description: b.description,
      })) || [],
    course_batch:
      apiResponse.data.courseBatch?.map((batch) => ({
        name: batch.name,
...
export async function getAllCourse(): Promise<CourseDetailResponse> {
  return apiGet<CourseDetailResponse>(`/courses?isDisplayed=true`, { revalidate: 60 });
}
...
export async function getAllContributors(): Promise<Contributor[]> {
  const response = await apiGet<ContributorsResponse>("/contributors?isDisplayed=true");
